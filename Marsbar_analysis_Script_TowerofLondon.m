%September 8, 2020
%DEAF COGNITION: Tower of London
%Barbara Manini
...
mainfolder_Data='/Users/deafneuralplasticitylab/Documents/MATLAB/1-First Level analysis/Lev1st_TowerofLondon/';
...
for n=1:size(subjs,1)
    ...
    %analysis folder
    P= dir(fullfile(subjroot ,'con_0005.nii')); % con= 0 1 0 0 tower of London condition% get images from the contrast directory where they are stored
    V = spm_vol(P);
    ...
    K= dir(fullfile(subjroot ,'con_0007.nii')); % con= 1 0 0 0 control condition % get images from contrast directory where they are stored
    J = spm_vol(K);
    ...
    [datamean_con datavar_con o_con] = summary_data(Ycon, 'mean'); %assign the mean extractions for each ROI and each image to variable called datamean 
     ...
     data_file= fullfile (roi_dir,'TowerofLondon_L_HG_roi.mat');
     save(data_file, 'datamean_tol', 'datamean_con','regionname'); %save the data in the given participant' directory
     
     clear subjroot roi_dir r L_HG_name roinames P V D D2 J K H H2 datamean_tol datavar_tol o_tol region_name datamean_con datavar_con o_con Ytol Wcon data_file
...
for n=1:size(subjs,1)
    ...
    P= dir(fullfile(subjroot ,'con_0005.nii')); % con= 0 1 0 0 tower of London condition% get images from the contrast directory where they are stored
    V = spm_vol(P);
%     tol_file  = strcat(subjroot ,'/',tol_name);
    D = strvcat(V.name); %load images into format for Marsbar  
    D2= strcat(subjroot,'/' , D);
    roi_array= maroi(fullfile(roinames));
    % Extract data
...
%   tol_file  = strcat(subjroot ,'/',tol_name);
    ...
    %regionname = char(region_name(Y)); %note the regionname   
     ...
     data_file= fullfile (roi_dir,'TowerofLondon_R_HG_roi.mat');
     save(data_file, 'datamean_tol', 'datamean_con','regionname'); %save the data in the given participant' directory
     
     clear subjroot roi_dir r R_HG_name roinames P V D D2 J K H H2 datamean_tol datavar_tol o_tol region_name datamean_con datavar_con o_con Ytol Wcon data_file
...
for n=1:size(subjs,1)
    ...
    [datamean_con datavar_con o_con] = summary_data(Ycon, 'mean'); %assign the mean extractions for each ROI and each image to variable called datamean 
     ...
     data_file= fullfile (roi_dir,'TowerofLondon_L_PT_roi.mat');
     save(data_file, 'datamean_tol', 'datamean_con','regionname'); %save the data in the given participant' directory
     
     clear subjroot roi_dir r L_PT_name roinames P V D D2 J K H H2 datamean_tol datavar_tol o_tol region_name datamean_con datavar_con o_con Ytol Wcon data_file
...
for n=1:size(subjs,1)
    ...
    [datamean_con datavar_con o_con] = summary_data(Ycon, 'mean'); %assign the mean extractions for each ROI and each image to variable called datamean 
     ...
     data_file= fullfile (roi_dir,'TowerofLondon_R_PT_roi.mat'); %save the data in the given participant' directory
     save(data_file, 'datamean_tol', 'datamean_con','regionname'); 
     
     clear subjroot r R_PT_name roinames P V D D2 J K H H2 datamean_tol datavar_tol o_tol region_name datamean_con datavar_con o_con Ytol Wcon data_file
...
for n=1:size(subjs,1)
    ...
    [datamean_con datavar_con o_con] = summary_data(Ycon, 'mean'); %assign the mean extractions for each ROI and each image to variable called datamean 
     ...
     data_file= fullfile (roi_dir,'TowerofLondon_L_STC_roi.mat');%save the data in the given participant' directory
     save(data_file, 'datamean_tol', 'datamean_con','regionname'); 
     
     clear subjroot r L_STC_name  stc_dir roinames P V D D2 J K H H2 datamean_tol datavar_tol o_tol region_name datamean_con datavar_con o_con Ytol Wcon data_file
...
for n=1:size(subjs,1)
    ...
    [datamean_con datavar_con o_con] = summary_data(Ycon, 'mean'); %assign the mean extractions for each ROI and each image to variable called datamean 
     ...
     data_file= fullfile (roi_dir,'TowerofLondon_R_STC_roi.mat'); %save the data in the given participant' directory
     save(data_file, 'datamean_tol', 'datamean_con','regionname'); 
     
     clear subjroot roi_dir r R_STC_name roinames P V D D2 J K H H2 datamean_tol datavar_tol o_tol region_name datamean_con datavar_con o_con Ytol Wcon data_file
...
datamean_tol_vector=zeros(6,length(subjs));
...
for n=1:size(subjs,1)
    for s=1:size(data_file,1)
       ...
       load (roi1);
       datamean_tol_vector (s,n) = datamean_tol;
...
filename = 'TowerofLondon_ROIdata.xlsx';

datamean_tol_vector=(datamean_tol_vector.');
datamean_tol_table=array2table(datamean_tol_vector);

datamean_con_vector=(datamean_con_vector.');
datamean_con_table=array2table(datamean_con_vector);

writetable(datamean_tol_table,filename,'Sheet',1,'Range','A1:F41')%range changes on the base of N subjs
writetable(datamean_con_table,filename,'Sheet',1,'Range','G1:L41')

%LIST OF ROI
%datamean_tol_vector1: Left Heschl's gyrus
%datamean_tol_vector2: Right Heschl's gyrus
%datamean_tol_vector3: Left planum temporale
%datamean_tol_vector4: Right planum temporale
%datamean_tol_vector5: Left superior temporal cortex
%datamean_tol_vector6: Right superior temporal cortex
